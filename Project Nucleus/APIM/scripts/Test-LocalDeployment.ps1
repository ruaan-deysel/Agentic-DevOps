...
param (
    ...
    [Parameter(Mandatory = $false)]
    [string]$ResourceGroupName = "rg-nucleus-apim-$Environment-local",
...
try {
    ...
    $paramFile = Join-Path $PSScriptRoot ".." "config" "ms.apim" "parameters.$Environment.bicepparam"
    ...
    if (-not $SkipDeployment) {
        if ($WhatIf) {
            ...
            if (-not $SkipPostDeployment) {
                ...
                & $postDeployScript -ResourceGroup $ResourceGroupName -Environment $Environment -LogPath $logDir
                if ($LASTEXITCODE -ne 0) {
                    Write-Error "Post-deployment configuration failed"
                    exit 1
                }
